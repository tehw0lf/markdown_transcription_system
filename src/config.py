...
class ConfigManager:
    ...
    # Read-only defaults; always consumed via .copy(), never mutated in place.
    ...
    def load_config(self, config_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            # Already descriptive (validation, unsupported format); don't re-wrap.
            raise
    ...
    def _validate_config(self):
        ...
        # Only warn for an explicitly configured path; the default is allowed
        # to not exist yet.
        if not vault_path.exists() and str(vault_path) != str(Path.home() / "Notes"):
            warnings.warn(
                f"Vault path does not exist: {vault_path}", stacklevel=2
            )
    ...
    def create_example_config(self, config_path: str, config_type: str = "obsidian"):
        ...
        try:
            ...
            raise ConfigurationError(
                f"Error creating example configuration: {e}"
            ) from e
